...
uchar4 tRGBA (float r, float g, float b, float a)
{
...
float4 vec_transform_point( float16 m, float4 v)
{
...
float4 vec_transform_vector( float16 m, float4 v)
{
...
void perform_integrator (int pIdx, float16 deltaPos, float4 gravity_impulse, float dt,
...
void perform_writeback (int vertindex, float16 mat, __global ushort* indices, __global float4* vertices, 
						__global Particle* particles, __global ParticleDebug* particlesDebug)
{
	...
	p.w = as_float(particlesDebug[particleindex].mColor);
	
	// vertices are XYZc  where c is a 4 byte integer colour
	vertices[vertindex] = p;
}

void perform_writeback_org (int vertindex, float16 mat, __global ushort* indices, __global float4* vertices, 
						__global Particle* particles, __global ParticleDebug* particlesDebug)
{
	// output vertex draws from particle specified by its corresponding entry in indices
	ushort particleindex = indices[vertindex];
...
void perform_distancesolve (int subIdx, float compressionTolerance, __global Particle *particles, __global ParticleDebug *particlesDebug, __global ConstraintOctet* octet)
{
	...
	void perform_distancesolve_tm_opt (int subIdx, float compressionTolerance, __global Particle *particles, __global ParticleDebug *particlesDebug, __global ConstraintOctet* octet)
	{
		const float Epsilon = 1e-5f;
      ...
      float mMin = octet->mMin[subIdx]; 
      float mMax = octet->mMax[subIdx]; 
      ...
      float4 p1Pos = p1->mPos; 
      float4 p2Pos = p2->mPos; 
		...
		if (mag > Epsilon)
		{
			if (mag < (mMin - compressionTolerance))
			{
				factor	= ((mMin - compressionTolerance) - mag) / (mag * (invMass1 + invMass2));
				repulse	=	p1p2;
				repulse	*=	invMass1 * factor;
				p1p2	*=  invMass2 * factor;

				if (!p1Locked)
					p1->mPos = p1Pos - repulse;
				else
					p1p2 += repulse;
			...
			{
				factor	= (mag - (mMax) ) / (mag * (invMass1 + invMass2));
	...
	void perform_distancesolve_tm_orig (int subIdx, float compressionTolerance, __global Particle *particles, __global ParticleDebug *particlesDebug, __global ConstraintOctet* octet)
	{
		const float Epsilon = 1e-5f;
...
void perform_distancesolve_atomic (int subIdx, float compressionTolerance, __global Particle *particles, __global ParticleDebug *particlesDebug, __global ConstraintOctet* octet, __global int* atomicLocks)
{
   const float Epsilon = 1e-5f;
   ...
   int processed = 0;
   while(!processed) {
      if(atom_cmpxchg(lock1,0,1) == 0) {
         if(atom_cmpxchg(lock2,0,1) == 0) {
...
void perform_driversolve (int subIdx, float effectScale, float verticalDistanceScale, float normalDistanceScale, float invVDS, float invNDS,
...
void perform_vector_integrator (float4 vdt_ratio, float4 acc, cl_int numparticles, int pIdx,
...
void perform_vector_writeback (uint numverts, uint vertindex, float16 mat, __global ushort* indices, __global Particle* particles, __global ParticleDebug* particlesDebug, __global float4* vertices)
{
...
void perform_vector_distancesolver (float compressionTolerance, __global ConstraintOctet* constraints, __global Particle* particles, __global ParticleDebug* particlesDebug)
{
...
void perform_vector_driversolver (float vertDistanceScale, float normDistanceScale, float invVertDistanceScale, float invNormDistanceScale, float effectScale,
...
__kernel void task_integrator (__global IntegratorState *iState, __global ConstraintTuningParameters *ctp, 
							   __global Particle *particles, __global ParticleDebug *particlesDebug, cl_int numparticles,
							   __global short *indices, __global uint *mapped, float16 deltaPos, float4 gravity, float dt)
{
...
__kernel void task_writeback (__global IntegratorState *iState, __global ConstraintTuningParameters *ctp, 
							  __global Particle *particles, __global ParticleDebug *particlesDebug, cl_int numparticles, 
							  float16 mat, __global float4 *vertices, __global ushort *indices, cl_int numverts)
{
...
__kernel void task_distancesolver (__global IntegratorState *iState, __global ConstraintTuningParameters *ctp, 
								   __global Particle *particles, __global ParticleDebug *particlesDebug, cl_int numparticles, 
...
__kernel void task_driversolver (__global IntegratorState *iState, __global ConstraintTuningParameters *ctp, 
								 __global Particle *particles, __global ParticleDebug *particlesDebug, cl_int numparticles, 
								 __global ConstraintOctet *octets, cl_int numoctets,
...
__kernel void task_spheresolver (__global IntegratorState *iState, __global ConstraintTuningParameters *ctp, 
								 __global Particle *particles, __global ParticleDebug *particlesDebug, cl_int numparticles, 
								 __global ConstraintOctet *octets, cl_int numoctets,
...
__kernel void task_tubesolver (__global IntegratorState *iState, __global ConstraintTuningParameters *ctp, 
							   __global Particle *particles, __global ParticleDebug *particlesDebug, cl_int numparticles, 
							   __global ConstraintOctet *octets, cl_int numoctets,
							   __global VolumeTransform *volumes)
{
...
__kernel void scalar_integrator (__global IntegratorState *iState, __global ConstraintTuningParameters *ctp, 
								 __global Particle *particles, __global ParticleDebug *particlesDebug, cl_int numparticles,
								 __global short *indices, __global uint *mapped, float16 deltaPos, float4 gravity, float dt)
{
	uint numstrands = get_global_size(0);
	uint pIdx = (uint)get_global_id(0);
	float4 acc = gravity * dt * dt;

	for (; pIdx < numparticles; pIdx += numstrands) {
		perform_integrator(pIdx, deltaPos, acc, dt, particles, particlesDebug, indices, iState, ctp);
	}
}

__kernel void scalar_integrator_split (__global IntegratorState *iState, __global ConstraintTuningParameters *ctp, 
								 __global Particle *particles, __global ParticleDebug *particlesDebug, cl_int numparticles,
								 __global short *indices, __global uint *mapped, float16 deltaPos, float4 gravity, float dt)
{
	uint numstrands = get_global_size(0);
...
__kernel void scalar_writeback (__global IntegratorState *iState, __global ConstraintTuningParameters *ctp, 
								__global Particle *particles, __global ParticleDebug *particlesDebug, cl_int numparticles, 
								float16 mat, __global float4 *vertices, __global ushort *indices, cl_int numverts)
{
	uint numstrands = get_global_size(0);
	uint vertindex = (uint)get_global_id(0);
	
	for (; vertindex < numverts; vertindex += numstrands) {
		perform_writeback(vertindex, mat, indices, vertices, particles, particlesDebug);
	}
}

__kernel void scalar_writeback_split (__global IntegratorState *iState, __global ConstraintTuningParameters *ctp, 
								__global Particle *particles, __global ParticleDebug *particlesDebug, cl_int numparticles, 
								float16 mat, __global float4 *vertices, __global ushort *indices, cl_int numverts)
{
	uint numstrands = get_global_size(0);
...
__kernel void scalar_distancesolver (__global IntegratorState *iState, __global ConstraintTuningParameters *ctp, 
									 __global Particle *particles, __global ParticleDebug *particlesDebug, cl_int numparticles, 
									 __global ConstraintOctet *octets, cl_int numoctets)
{
	float comp = ctp->mCompression;
...
#ifdef TM
	__kernel void scalar_distancesolver_tm (__global IntegratorState *iState, __global ConstraintTuningParameters *ctp,
										 __global Particle *particles, __global ParticleDebug *particlesDebug, cl_int numparticles,
										 __global ConstraintOctet *octets, cl_int numoctets)
	{
		float comp = ctp->mCompression;
		...
		if(get_global_id(0) < numoctets * 8) {
			...
			#ifdef TMOPT
				perform_distancesolve_tm_opt(slice, comp, particles, particlesDebug, &octets[octet]);
			#else
				perform_distancesolve_tm_orig(slice, comp, particles, particlesDebug, &octets[octet]);
			#endif
		}
	}
#endif

__kernel void scalar_distancesolver_atomic (__global IntegratorState *iState, __global ConstraintTuningParameters *ctp,
                            __global Particle *particles, __global ParticleDebug *particlesDebug, cl_int numparticles,
                            __global ConstraintOctet *octets, cl_int numoctets, __global int* atomicLocks)
{
   float comp = ctp->mCompression;
   int octet, slice;
...
__kernel void scalar_driversolver (__global IntegratorState *iState, __global ConstraintTuningParameters *ctp, 
								   __global Particle *particles, __global ParticleDebug *particlesDebug, cl_int numparticles, 
								   __global ConstraintOctet *octets, cl_int numoctets,
								   __global PosNormPair *drivers)
{
	...
	for (; cIdx < numoctets * 8; cIdx += numstrands) {
		int slice = cIdx & 7;
...
__kernel void scalar_driversolver_split (__global IntegratorState *iState, __global ConstraintTuningParameters *ctp, 
								   __global Particle *particles, __global ParticleDebug *particlesDebug, cl_int numparticles, 
								   __global ConstraintOctet *octets, cl_int numoctets,
								   __global PosNormPair *drivers)
{
...
__kernel void scalar_spheresolver (__global IntegratorState *iState, __global ConstraintTuningParameters *ctp, 
								   __global Particle *particles, __global ParticleDebug *particlesDebug, cl_int numparticles, 
								   __global ConstraintOctet *octets, cl_int numoctets,
								   __global VolumeTransform *volumes)
{
...
__kernel void scalar_tubesolver (__global IntegratorState *iState, __global ConstraintTuningParameters *ctp, 
								 __global Particle *particles, __global ParticleDebug *particlesDebug, cl_int numparticles, 
								 __global ConstraintOctet *octets, cl_int numoctets,
								 __global VolumeTransform *volumes)
{
...
__kernel void vector_integrator (__global IntegratorState *iState, __global ConstraintTuningParameters *ctp, 
								 __global Particle *particles, __global ParticleDebug *particlesDebug, cl_int numparticles,
								 __global short *indices, __global uint *mapped, float16 deltaPos, float4 gravity, float dt)
{
	float4 acc = gravity * dt * dt;
...
__kernel void vector_writeback (__global IntegratorState *iState, __global ConstraintTuningParameters *ctp, 
								__global Particle *particles, __global ParticleDebug *particlesDebug, cl_int numparticles, 
								float16 mat, __global float4 *vertices, __global ushort *indices, cl_int numverts)
{
	if (USE_TASK_VECTOR && (get_global_size(0) == 1))
...
__kernel void vector_distancesolver (__global IntegratorState *iState, __global ConstraintTuningParameters *ctp, 
									 __global Particle *particles, __global ParticleDebug *particlesDebug, cl_int numparticles, 
									 __global ConstraintOctet *octets, cl_int numoctets)
{
