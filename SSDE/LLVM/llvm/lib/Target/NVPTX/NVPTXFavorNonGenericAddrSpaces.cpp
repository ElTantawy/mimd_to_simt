...
private:
  ...
  Value *hoistAddrSpaceCastFrom(Value *V, std::vector<Value *>* Visited,int Depth = 0);
  /// Helper function for GEPs.
  Value *hoistAddrSpaceCastFromGEP(GEPOperator *GEP, std::vector<Value *>* Visited, int Depth);
  /// Helper function for bitcasts.
  Value *hoistAddrSpaceCastFromBitCast(BitCastOperator *BC, std::vector<Value *>* Visited, int Depth);
  /// Helper function for phinodes.
  Value *hoistAddrSpaceCastFromPHINode(PHINode *PHI, std::vector<Value *>* Visited,int Depth);
...
Value *NVPTXFavorNonGenericAddrSpaces::hoistAddrSpaceCastFromGEP(
    GEPOperator *GEP,  std::vector<Value *>* Visited, int Depth) {
  Visited->push_back(dyn_cast<Value>(GEP));
  Value *NewOperand =
      hoistAddrSpaceCastFrom(GEP->getPointerOperand(), Visited, Depth + 1);
  ...
  if (Instruction *GEPI = dyn_cast<Instruction>(GEP)) {
    ...
    GetElementPtrInst *NewGEP = GetElementPtrInst::Create(Cast->getOperand(0), Indices,
        "", GEPI);
...
Value *NVPTXFavorNonGenericAddrSpaces::hoistAddrSpaceCastFromBitCast(
    BitCastOperator *BC,  std::vector<Value *>* Visited, int Depth) {
  Visited->push_back(dyn_cast<Value>(BC));
  Value *NewOperand = hoistAddrSpaceCastFrom(BC->getOperand(0),Visited, Depth + 1);
  if (NewOperand == nullptr)
    return nullptr;
...
Value *NVPTXFavorNonGenericAddrSpaces::hoistAddrSpaceCastFromPHINode(
    PHINode *PH, std::vector<Value *> * Visited,int Depth) {
	  Visited->push_back(dyn_cast<Value>(PH));
	  Type * newPHIType = nullptr;
	  for(unsigned i=0; i<PH->getNumIncomingValues();i++){
		  if(std::find(Visited->begin(),Visited->end(),PH->getIncomingValue(i))!=Visited->end()) continue;
			  Value *NewOperand = hoistAddrSpaceCastFrom(PH->getIncomingValue(i),Visited, Depth + 1);
			  if (NewOperand == nullptr)
				  return nullptr;
			  newPHIType = dyn_cast<Operator>(NewOperand)->getOperand(0)->getType();
	  }
	  if(newPHIType==nullptr){
		  return nullptr;
	  }
	  Value *NewASC;
	  ...
	  for(unsigned i=0; i<PH->getNumIncomingValues();i++){
		  ...
		  NewPH->addIncoming(ASC->getOperand(0),PH->getIncomingBlock(i));
	  }
	  NewASC = new AddrSpaceCastInst(NewPH, PH->getType(), "", PH->getParent()->getFirstNonPHI());
	  NewASC->takeName(PH);
	  PH->replaceAllUsesWith(NewASC);
...
Value *NVPTXFavorNonGenericAddrSpaces::hoistAddrSpaceCastFrom(Value *V,
		 std::vector<Value *>* Visited, int Depth) {
  ...
  if (GEPOperator *GEP = dyn_cast<GEPOperator>(V))
    return hoistAddrSpaceCastFromGEP(GEP, Visited, Depth);

  if (BitCastOperator *BC = dyn_cast<BitCastOperator>(V))
    return hoistAddrSpaceCastFromBitCast(BC, Visited, Depth);

  if (PHINode *PH = dyn_cast<PHINode>(V))
    return hoistAddrSpaceCastFromPHINode(PH, Visited, Depth);
...
bool NVPTXFavorNonGenericAddrSpaces::optimizeMemoryInstruction(Instruction *MI,
                                                               unsigned Idx) {
  std::vector<Value*> * Visited = new std::vector<Value*>;
  Visited->clear();
  Value *NewOperand = hoistAddrSpaceCastFrom(MI->getOperand(Idx),Visited);
  if (NewOperand == nullptr)
    return false;
