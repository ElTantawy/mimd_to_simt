...
void NVPTXLowerKernelArgs::handlePointerParam(Argument *Arg) {
  ...
  if(Arg->getType()->getPointerAddressSpace()==ADDRESS_SPACE_GENERIC){
	  Instruction *FirstInst = Arg->getParent()->getEntryBlock().begin();
	  Instruction *ArgInGlobal = new AddrSpaceCastInst(Arg,
...
bool NVPTXLowerKernelArgs::runOnFunction(Function &F) {
  ...
  for (Argument &Arg : F.args()) {
    if (Arg.getType()->isPointerTy()) {
      ...
      else /*if (TM && TM->getSubtarget<NVPTXSubtarget>().getDrvInterface() == NVPTX::CUDA)*/
